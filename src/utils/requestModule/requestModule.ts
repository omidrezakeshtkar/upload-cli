...
export const importUserData = async (jsonarray: any[]) => {
	...
	const usernameLists: any = await getUserNameList(requestDataArray);
	const duplicateUsernameList = lodash.intersection(
		usernameLists.usernameList,
		usernameLists.csvFileUsernameList
	);
	if (!lodash.isEmpty(usernameLists.usernameList))
		lodash.remove(requestDataArray, (list) =>
			duplicateUsernameList.includes((list as any).username)
		);
	const batchUsers = lodash.chunk(
		requestDataArray,
	...
	if (duplicateUsernameList.length > 0) {
		progressBar.stop();
		warningLog("duplicate users: ", duplicateUsernameList.join(","));
	}
